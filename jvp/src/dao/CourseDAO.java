...
public class CourseDAO {
    ...
    // Get courses actually enrolled by a specific student (used in StudentUI)
    public List<Course> getEnrolledCourses(int studentId) {
        List<Course> courses = new ArrayList<>();
        // Try student_courses enrollment table first
        String query = "SELECT c.*, u.full_name as lecturer_name FROM courses c " +
                "INNER JOIN student_courses sc ON c.course_id = sc.course_id " +
                "LEFT JOIN lecturers l ON c.lecturer_id = l.lecturer_id " +
                "LEFT JOIN users u ON l.user_id = u.user_id " +
        ...
        // Fallback: if no enrollment records exist, fall back to semester-based lookup
        if (courses.isEmpty()) {
            return getStudentCoursesFallback(studentId);
        }
        return courses;
    }

    // Fallback: get courses by student's current semester (used when student_courses table is empty)
    private List<Course> getStudentCoursesFallback(int studentId) {
        List<Course> courses = new ArrayList<>();
        String query = "SELECT c.*, u.full_name as lecturer_name FROM courses c " +
                "LEFT JOIN lecturers l ON c.lecturer_id = l.lecturer_id " +
                "LEFT JOIN users u ON l.user_id = u.user_id " +
                "WHERE c.semester = (SELECT current_semester FROM students WHERE student_id = ?)";
        try (PreparedStatement pstmt = DatabaseConnection.getConnection().prepareStatement(query)) {
            pstmt.setInt(1, studentId);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
