...
public class AttendanceDAO {
    ...
    public double getAttendancePercentageWithMedical(int studentId, int courseId, String sessionType) {
        ...
        // First get medical dates for this student
        String medicalQuery = "SELECT medical_date FROM medicals WHERE student_id = ? AND status = 'approved'";
        List<java.sql.Date> medicalDates = new ArrayList<>();
        try (PreparedStatement pstmt = conn.prepareStatement(medicalQuery)) {
            pstmt.setInt(1, studentId);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                medicalDates.add(rs.getDate("medical_date"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(query)) {
            ...
            while (rs.next()) {
                ...
                // Check if this date has an approved medical
                boolean hasMedical = medicalDates.contains(sessionDate);
